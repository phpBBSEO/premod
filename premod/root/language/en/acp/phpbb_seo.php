...
$lang = array_merge($lang, array(
	...
	'sql_rewrite_explain' => 'This option will allow you to choose url for each topic. You will be able to accurately set topic url when posting new topic or when editing an existing one. This functionality is though limited to forum admins. <br/><br/><b style="color:red">Please Note :</b><br/><ul style="margin-left:20px">If turning on this option will not change topic urls in advanced mode, existing urls being then stored as they are displayed in the data base, it’s not the case in mixed and simple modes.<br/>URLs such as <b>topicxx.html</b> will be redirected to <b>topic-txx.html</b> in order to allow custom title injection usage upon demand.<br/>This implies many redirection and should thus be activated only after you thought about it twice and tested it where it can cause no trouble. If you do not want any keyword injection in topic urls, just do not activate this option.<br/><br/>The feature also has the great advantage to fasten the url rewriting by a lot when using the virtual folder option, and to make it a lot easier to retrieve rewritten urls from any page.</ul>',
